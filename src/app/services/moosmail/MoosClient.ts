...
export class MoosClient {
  ...
  constructor(public name: string, public address: string, private password?: string) {
    ...
    this.ws.addEventListener('message', (evt => {
      if (MoosmailService.pauseUpdates) { return; }
      const origString = evt.data;
      const mailName = origString.split('=')[0];

      const mail = this.receivedMail.get(mailName) || new MoosMail();
      mail.name = mailName;
      ...
      if (mail.notifyOnUpdate) {
        // TODO Re-add local notifications from https://github.com/phonegap/phonegap-plugin-local-notification to https://ionicframework.com/docs/native/local-notifications
        /*
  ...
  sendMessage(name: string, content: string) {
    const mail = this.receivedMail.get(name) || new MoosMail();
    mail.name = name;
...
export class MoosMail {
  ...
  public hiddenFromList = false; // Users can unhide by resubscribing under the client card
  public expandOnList = false;
  public notifyOnUpdate = false;

  public newMessage: Observable<this>;
}
